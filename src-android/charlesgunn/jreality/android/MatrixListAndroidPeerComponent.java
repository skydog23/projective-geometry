 ...
 *
 */
public class MatrixListAndroidPeerComponent extends AndroidPeerComponent {
	...
	public MatrixListAndroidPeerComponent()	{
		super();
		instanceCount++;
	}
	
	public MatrixListAndroidPeerComponent(SceneGraphPath sgp, AndroidPeerComponent p, AndroidRenderer jr)		{
		super(sgp, p, jr);
		instanceCount++;
	}
...
//					jr.globalGL.glNewList(displayList, GL11.GL_COMPILE);
...
//			jr.globalGL.glCallList(displayList);
	...
	protected boolean someSubNodeIsDirty()	{
		...
		AndroidPeerComponent child = children.get(0);
		return (child.someSubNodeIsDirty());
	}
	private void printUpState()	{
		if (debug) theLog.info("dld\tcld:\t"+displayListDirty+"\t"+"\t"+name);
		if (parent!=null)((MatrixListAndroidPeerComponent) parent).printUpState();
	}
	int metric;
	double[] o2c, o2ndc;
	...
	protected void renderChildren() {
		if (isCopyCat)		{
			...
			int count = 0;
			MatrixListAndroidPeerComponent child = (MatrixListAndroidPeerComponent) children.get(0);
			...
			for (int j = 0; j<nn; ++j)	{
				...
				if (cumulativeIsReflection != jr.renderingState.flipped)	{
					jr.globalGL.glFrontFace(cumulativeIsReflection ? GL11.GL_CW : GL11.GL_CCW);
...
//				System.err.println("Setting up final thing");
	...
	private boolean existsHigherCat()	{
		if (parent != null) 
			if (((MatrixListAndroidPeerComponent) parent).isCopyCat) return true;
			else return ((MatrixListAndroidPeerComponent)parent).existsHigherCat();
		else return false;
	}
	@Override
