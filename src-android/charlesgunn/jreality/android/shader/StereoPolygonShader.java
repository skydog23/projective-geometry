...
public class StereoPolygonShader extends DefaultPolygonShader {
	...
	AndroidTexture2D joglLeft, joglRight;
	public void  setFromEffectiveAppearance(EffectiveAppearance eap, String name)	{
		...
		if (AttributeEntityUtility.hasAttributeEntity(Texture2D.class, 
			...
			leftTexture2D = (Texture2D) AttributeEntityUtility.createAttributeEntity(Texture2D.class, 
				...
				joglLeft = new AndroidTexture2D(leftTexture2D);
		}
		if (AttributeEntityUtility.hasAttributeEntity(Texture2D.class, 
			...
			rightTexture2D = (Texture2D) AttributeEntityUtility.createAttributeEntity(Texture2D.class, 
					ShaderUtility.nameSpace(name,CommonAttributes.TEXTURE_2D+".right"), eap);			
		    	//LoggingSystem.getLogger(this).fine("Got texture 2d for eap "+((Appearance) eap.getAppearanceHierarchy().get(0)).getName());
				joglRight = new AndroidTexture2D(rightTexture2D);
		}
	}
	@Override
