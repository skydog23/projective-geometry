...
import charlesgunn.jreality.android.shader.DefaultGeometryShader;
...
public class AndroidPeerComponent extends AndroidPeerNode implements TransformationListener, AppearanceListener,SceneGraphComponentListener {

	protected EffectiveAppearance eAp;
	protected Vector<AndroidPeerComponent> children;
	protected AndroidPeerComponent parent;
	...
	final AndroidPeerComponent self = this;
	...
	public AndroidPeerComponent()	{
		super();
	}

	public AndroidPeerComponent(SceneGraphPath sgp, AndroidPeerComponent p, AndroidRenderer jr)		{
		super();
		init(sgp, p, jr);
	}
	
	public void init(SceneGraphPath sgp, AndroidPeerComponent p, AndroidRenderer jr) {
		init(null, sgp, p, jr);
	}

	public void init(GoBetween gb, SceneGraphPath sgp, AndroidPeerComponent p, AndroidRenderer jr) {
		this.jr = jr;
		...
		goBetween.addAndroidPeer(this);
		name = "AndroidPeer:"+goBetween.originalComponent.getName();
		children = new Vector<AndroidPeerComponent>();
		parent = p;
	...
	public void dispose()	{
		...
		for (AndroidPeerComponent child : children) {
			child.dispose();
		}	
		children.clear();
		goBetween.removeAndroidPeer(this);
	}
...
//			System.err.println("AndroidPC: flipped = "+jr.renderingState.flipped);
		...
		if (oldFlipped != jr.renderingState.flipped) {
			jr.globalGL.glFrontFace(jr.renderingState.flipped ? GL11.GL_CW : GL11.GL_CCW);
		}
		...
		if (goBetween.peerGeometry != null && goBetween.peerGeometry.localClippingPlane)	{
			AndroidRendererHelper.pushClippingPlane(jr, (float[]) null);
		}
...
//		System.err.println("AndroidPC: child count = "+n);
		childlock.readLock();
		for (AndroidPeerComponent child : children)	{	
			child.render();
		}
	...
	private void postRender() {
		if (goBetween.peerGeometry != null && goBetween.peerGeometry.localClippingPlane)	{
			AndroidRendererHelper.popClippingPlane(jr);
		}
		if (rhInfo != null && rhInfo.hasSomeActiveField)	{
			jr.rhStack.pop();
	...
	protected void pushTransformation(double[] m) {
		if ( jr.stackDepth < AndroidRenderer.MAX_STACK_DEPTH) {
			jr.globalGL.glPushMatrix();
			jr.globalGL.glMultMatrixf(Util.getTransposedFloatMatrix(null,m),0);
		}
		else {
//			System.err.println("o2c: "+Rn.matrixToString(jr.context.getObjectToCamera()));
			int stackCounter = jr.stackDepth - AndroidRenderer.MAX_STACK_DEPTH;
			if (stackCounter == 0)	{
...
//			jr.globalGL.glLoadTransposeMatrixd(jr.context.getObjectToCamera(),0);	
			jr.globalGL.glLoadMatrixf(Util.getTransposedFloatMatrix(null,jr.matrixStack[stackCounter].getArray()),0);	
		}
		jr.stackDepth++;
	}
	...
	protected void setIndexOfChildren()	{
		...
		for (int i = 0; i<n; ++i)	{
			...
			AndroidPeerComponent jpc = getPeerForChildComponent(sgc);
			if (jpc == null)	{
	...
	private AndroidPeerComponent getPeerForChildComponent(SceneGraphComponent sgc) {
//		childlock.readLock();
		for (AndroidPeerComponent jpc : children)	{
...
//		childlock.readUnlock();
	...
	public void appearanceChanged(AppearanceEvent ev) {
		//LoggingSystem.getLogger(this).finer("AndroidPeerComponent: appearance changed: "+goBetween.getOriginalComponent().getName());
...
//			LoggingSystem.getLogger(this).fine("handle new appearance "+name);
	...
	protected void propagateAppearanceChanged()	{
		if (debug) LoggingSystem.getLogger(this).finer("AndroidPeerComponent: propagate: "+name);
		appearanceDirty = true;
		for (AndroidPeerComponent child : children) {
	...
	protected void propagateEffectiveAppearanceChanged()	{
		if (debug) LoggingSystem.getLogger(this).finer("AndroidPeerComponent: propagateeap: "+name);
		appearanceDirty =  true;
		effectiveAppearanceDirty = true;
		for (AndroidPeerComponent child : children) {
...
//	    	LoggingSystem.getLogger(this).info("Updating shader for "+getName());
	...
	public void childAdded(SceneGraphComponentEvent ev) {
		if (debug) theLog.finer("AndroidPeerComponent: Container Child added to: "+name);
		...
		case SceneGraphComponentEvent.CHILD_TYPE_COMPONENT:
			...
			AndroidPeerComponent pc = ConstructPeerGraphVisitor.constructPeerForSceneGraphComponent(sgc, this, jr);
			childlock.writeLock();
			children.add(pc);						
