...
public class AndroidPeerGeometry extends AndroidPeerNode	implements GeometryListener{
	...
	public AndroidPeerGeometry(Geometry g, AndroidRenderer jr)	{
		super(jr);
		originalGeometry = g;
		name = "AndroidPeer:"+g.getName();
	...
	public void render(AndroidPeerComponent jpc) {
		...
		if (geometryShader.isVertexDraw() && hasPointLabels) {
			AndroidRendererHelper.drawPointLabels(jr, ps,  jpc.geometryShader.pointShader.getTextShader());
		}
		if (geometryShader.isEdgeDraw() && hasEdgeLabels) {
			AndroidRendererHelper.drawEdgeLabels(jr, ils, jpc.geometryShader.lineShader.getTextShader());
		}
		if (geometryShader.isFaceDraw() && hasFaceLabels) {
			AndroidRendererHelper.drawFaceLabels(jr, ifs,  jpc.geometryShader.polygonShader.getTextShader());
		}
	}
