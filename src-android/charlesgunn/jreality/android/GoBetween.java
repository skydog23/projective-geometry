...
public class GoBetween extends AndroidPeerNode implements 
	GeometryListener, 
	TransformationListener, 
	AppearanceListener,
	SceneGraphComponentListener, LightListener	{
	SceneGraphComponent originalComponent;
	ArrayList<AndroidPeerComponent> peers = new ArrayList<AndroidPeerComponent>();
	AndroidPeerGeometry peerGeometry;
	Lock peersLock = new Lock();
	boolean singlePeer = false;

	protected static WeakHashMap<AndroidRenderer, HashMap> tableForJR = new WeakHashMap<AndroidRenderer, HashMap>();
	...
	protected static WeakHashMap<AndroidRenderer, HashMap<SceneGraphComponent, GoBetween>> rendererTable = 
		new WeakHashMap<AndroidRenderer, HashMap<SceneGraphComponent, GoBetween>>();
	
	public   static GoBetween goBetweenFor(AndroidRenderer jr, SceneGraphComponent sgc, boolean singlePeer)	{
...
//		System.err.println("Already have gb for "+sgc.getName());
	...
	protected GoBetween(SceneGraphComponent sgc, AndroidRenderer jr, boolean inheritSinglePeer)	{
		super();
		init(sgc, jr, inheritSinglePeer);
	}
	
	public void init (SceneGraphComponent sgc, AndroidRenderer jr, boolean inheritSinglePeer)	{
		this.jr = jr;
...
//		System.err.println("Initializing peer for "+originalComponent.getName());
		if (originalComponent.getGeometry() != null)  {
			peerGeometry = jr.geometryGB.getAndroidPeerGeometryFor(originalComponent.getGeometry());
...
//			System.err.println("removing go between for "+originalComponent.getName());
	...
	public AndroidPeerComponent getSinglePeer()	{
		return (peers.size() == 0 ? null : peers.get(0));
	}

	public void addAndroidPeer(AndroidPeerComponent jpc)	{
	...
	public void removeAndroidPeer(AndroidPeerComponent jpc)	{
		if (!peers.contains(jpc)) return;
		peersLock.writeLock();
		peers.remove(jpc);
		peersLock.writeUnlock();
	...
	public AndroidPeerGeometry getPeerGeometry() {
		return peerGeometry;
	}
...
//		peersLock.readLock();
		...
		for ( AndroidPeerComponent peer: peers)	{
...
//		peersLock.readUnlock();
	...
	public void appearanceChanged(AppearanceEvent ev) {
		...
		if (key.indexOf(CommonAttributes.IMPLODE_FACTOR) != -1 ) changed |= (AndroidPeerComponent.FACES_CHANGED);
		else if (key.indexOf(CommonAttributes.TRANSPARENCY) != -1) changed |= (AndroidPeerComponent.ALL_GEOMETRY_CHANGED);
		else if (key.indexOf(CommonAttributes.SMOOTH_SHADING) != -1) changed |= (AndroidPeerComponent.ALL_GEOMETRY_CHANGED);
		else if (key.indexOf(CommonAttributes.TUBE_RADIUS) != -1) changed |= (AndroidPeerComponent.LINES_CHANGED);
		else if (key.indexOf(CommonAttributes.POINT_RADIUS) != -1) changed |= (AndroidPeerComponent.POINTS_CHANGED);
		else if (key.indexOf(CommonAttributes.ANY_DISPLAY_LISTS) != -1) changed |= (AndroidPeerComponent.ALL_GEOMETRY_CHANGED);
		else if (key.endsWith("Shader")) changed |= AndroidPeerComponent.ALL_SHADERS_CHANGED;
		else if (key.endsWith("Shadername")) changed |= AndroidPeerComponent.ALL_SHADERS_CHANGED;
		// there are some appearances which we know aren't inherited, so don't propagate change event.
		else if (key.indexOf(CommonAttributes.TEXTURE_2D) != -1) changed |= (AndroidPeerComponent.FACES_CHANGED);
