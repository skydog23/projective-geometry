...
//	double a = 1.5, b = .75, c = 2.0;
	...
	public SceneGraphComponent getContent() {
		...
		circleSGC = SceneGraphUtility.createFullSceneGraphComponent("circle");
		...
		backWorld = SceneGraphUtility.createFullSceneGraphComponent("back world");
		bigWorld = SceneGraphUtility.createFullSceneGraphComponent("big world");
		...
		faceSGC.addChildren(dotsSGC, circleSGC);
...
//		regularPolygonFactory.setGenerateEdgesFromFaces(true);
		...
		Matrix m = new Matrix(MatrixBuilder.euclidean().scale(scaleFactor).getArray());
...
//		doBoundary();
		...
		circleSGC.setGeometry(IndexedLineSetUtility.circle(100));
		ap = circleSGC.getAppearance();
		...
		ap.setAttribute("pointShader."+CommonAttributes.POINT_SIZE, 1.2);
		...
		ap.setAttribute(DualizeSceneGraph.DUALIZE_POINTS, true);
		ap.setAttribute(DualizeSceneGraph.DUALIZE_LINES, false);
		MatrixBuilder.euclidean().scale(scaleFactor).assignTo(circleSGC);
		...
		double[] lai = {1,0,0,0,0,0};
		SceneGraphComponent laiSGC = LineUtility.sceneGraphForLine(null, lai, null, 1.0, false);
		laiSGC.setName("line at infinity");
		world.addChild(laiSGC);
		laiSGC.setAppearance(new Appearance());
		ap = laiSGC.getAppearance();
		ap.setAttribute("lineShader.diffuseColor", Color.black);
		ap.setAttribute("lineShader.drawTubes", false);
		ap.setAttribute("lineShader.lineWidth", 4.0);

		eucSGC.getAppearance().setAttribute(DualizeSceneGraph.FAN_RADIUS, .3);
		DualizeSceneGraph.setMetric(1);
...
//		ap.setAttribute("lineShader.lineWidth", 1.0);
		...
		bigWorld.addChildren(world, backWorld);
		backWorld.addChild(world);
//		polarSGC3.addChild(polarSGC2);
		double[] mat = Rn.diagonalMatrix(null, new double[]{-1,-1,-1,-1});
		new Matrix(mat).assignTo(backWorld);
		world.getAppearance().setAttribute(CommonAttributes.LINE_WIDTH, 3.5);
		return bigWorld;
	}
	...
	private void doDots()	{
		if (doRandom)	{
			...
			for (int i = 0; i<numRandomPoints; ++i)	{
				...
				double[] tmp = null;
				do {
					tmp = new double[]{Math.random()*2-1, Math.random()*2-1, 1};
					inside = true;
				...
				randomPoints[i] = new double[]{tmp[0], tmp[1],0,tmp[2]};
				dotColors[i] = new Color( ((int) (128+127*tmp[0])), ((int) (128+127 * tmp[1])), 0);
			}
		} else if (doShrink){ 
			numRandomPoints = 1+numSides*(numPlops);
			randomPoints = new double[numRandomPoints][4];
			dotColors = new Color[numRandomPoints];
			int count = 0;
			...
			for (int i = 0; i<numSides; ++i)	{
				...
				for (int j = 0 ; j<numPlops; ++j)	{
					...
					randomPoints[count] = Rn.copy(null, vs);
					randomPoints[count][3] = j+.001;
					count++;
				}
			}
...
//			polarGC.addChild(child);
	...
	public Component getInspector() {
		final TextSlider<Integer> nSlider = new TextSlider.Integer("num",SwingConstants.HORIZONTAL, 1, 20, numPlops);
		nSlider.addActionListener(new ActionListener() {
			
			@Override
			public void actionPerformed(ActionEvent arg0) {
				numPlops = nSlider.getValue().intValue();
				update();

			}
		});
		inspector.add(nSlider);
		final JCheckBox randomCB = new JCheckBox("random dots");
		randomCB.setSelected(doRandom);
		randomCB.addActionListener(new ActionListener() {
			
			@Override
			public void actionPerformed(ActionEvent e) {
				doRandom = randomCB.isSelected();
				update();
			}
		});
		inspector.add(randomCB);
		final JCheckBox shrinkCB = new JCheckBox("perspective dots");
		shrinkCB.setSelected(doShrink);
		shrinkCB.addActionListener(new ActionListener() {
			
			@Override
			public void actionPerformed(ActionEvent e) {
				doShrink = shrinkCB.isSelected();
				update();
			}
		});
		inspector.add(shrinkCB);
		return inspector;
	}
...
//		cam.setPerspective(false);
	...
	private void update() {
		doDots();
		polarSGC = DualizeSceneGraph.dualize(new SceneGraphPath(world,eucSGC));
		Appearance ap = polarSGC.getAppearance();
		ap.setAttribute(BOUNDING_BOX, Rectangle3D.EMPTY_BOX);
//				ap.setAttribute("lineShader.lineWidth", 1.0);
		polarSGC2.removeAllChildren();
		polarSGC2.addChild(polarSGC);
	}
	public static void main(String[] args) {
		new DualEuclideanHexaDemo().display();
	}
}
