...
public class AmesRoom extends Assignment {
	double[] abc = {0,0,.3};
...
//		tex2d.setTextureMatrix(texm);
		...
		tex2d.setTextureMatrix(texm);
		room.addChildren(frontwallSGC, floorSGC);
		frontwallSGC.getAppearance().setAttribute(CommonAttributes.EDGE_DRAW, false);
		...
		bothroom.getAppearance().setAttribute("lineShader.textShader."+CommonAttributes.TEXT_SCALE, .006);
...
//		theAmesRoom.setVertexAttributes(Attribute.COORDINATES, vertsDL);
		...
		double[] ideal2 = Rn.times(null, globalScale, Pn.dehomogenize(null, Rn.matrixTimesVector(null, fooM, ideal)));
		System.err.println("Image of ideal point = "+Rn.toString(ideal2));
...
//	}
	...
	protected SceneGraphComponent frontWallScene()	{
		...
		ifsf.setFaceColors(new Color[]{ window, window});//,border});
		ifsf.update();
