...
public abstract class AbstractDeformation {
	...
	public  double[][] valueAt(double[][] input, double[][] output) {
	 	  if (output == null || output.length != input.length || output[0].length != input[0].length)
	 		  output = new double[input.length][input[0].length];
	 	   for (int i = 0; i<input.length; ++i)	{
	 	   		valueAt(input[i], output[i]);
	 	   }
	 	   return output;
	}
	public static SceneGraphComponent deform(final SceneGraphComponent sgc, final AbstractDeformation deform)	{
	    ...
	    SceneGraphVisitor v =new SceneGraphVisitor() {
	        ...
	        public void visit(Sphere s)	{
	        	    LoggingSystem.getLogger(GeometryUtility.class).log(Level.WARNING, "Can't deform a sphere");
	        }
	    };
		v.visit(sgc);
        Set keys = map.keySet();
